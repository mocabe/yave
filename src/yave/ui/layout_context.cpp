...
namespace yave::ui {
  ...
  private:
    ...
    {
      if (w.has_parent()) {
        // TODO: avoid always using parent when layout of children will not
        // affect updated layout. (how can we handle rendering stage then?)
        auto& p =
          w.parent().id() == w.window_manager().root().id() ? w : w.parent();
