  ...
  namespace {
      ...
      message_map& msgs) -> monad
    {
      ...
      auto rec_g = [&](
        ...
        // check interface
        check_n(g, os, ng, msgs, m);

        // check input
        for (auto&& c : ng.input_connections(g)) {
          auto ci = ng.get_info(c);
          rec_n(ci->src_node(), ci->src_socket(), ng, msgs, m);
        }
      ...
      auto rec_n = [&](
                     ...
                     memo& m) -> void {
        if (ng.is_group(n))
          return rec_g(self, n, os, ng, msgs, m);

        if (ng.is_function(n))
          return rec_f(self, n, os, ng, msgs, m);

        if (ng.is_group_input(n))
          return check_io(n, os, ng, msgs);
      ...
      rec(out_node, out_socket, ng, msgs, m);
